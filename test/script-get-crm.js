import http from "k6/http";
import { htmlReport } from "./bundle.js";
import { check, sleep } from "k6";
//import { htmlReport } from 'https://raw.githubusercontent.com/benc-uk/k6-reporter/main/dist/bundle.js';

export const options = {
  vus: 10, // 10 usuários virtuais
  duration: "15s", // duração total do teste
  insecureSkipTLSVerify: true,
  cloud: {
    projectID: "3768735",
    // Test runs with the same name groups test runs together
    name: "Testes de GET CRM",
  },
};

export default function () {
  const token =
    "eyJhbGciOiJSUzI1NiIsInR5cCIgOiAiSldUIiwia2lkIiA6ICItT3FMWGtCcWNUSWRUN05jMHN1bkxWcE9NRUFESWpVMFZtRmJfUDBEdGY0In0..bi6MTLC13UC77fO8WKEE7O2_OPwFu7uCDPvAXXtUMo62UugI2NU-FQX7jDzK88XGVUTNQV6XqrbIdOuZ8UKmERcwLlS1a-ohRea6CRTlTdFqrBQY9PPbHn3JL1UiF0T7HNcx-HoFNfDrP5cU73RTEHWd1hb8fb3tRluoGlGGtEDD7sidZ7PVfhXO9w7R4M_f3veG-ZSLVGmc5JL5hkOVkqmNkM3zYf3RTIMS8NEVbimPz5DJ3uKOVvescib0TnU520-rXIomlsTLw-UMKnXKxoxkRPL0R1Fst0ZVxBUBwmoL8VbWEpzKXlJMF4G0l3qLgKfT65lVg89xEiqrldN52A";

  // Array de objetos contendo o nome e a URL de cada requisição
  const requests = [
    {
      name: "Listar Ofertas",
      url: "https://api-teste.ieztelecom.net.br/api/v1/crm/ofertas",
    },
    {
      name: "Listar Fabricantes",
      url: "https://api-teste.ieztelecom.net.br/api/v1/crm/aparelhos/fabricantes",
    },
    {
      name: "Listar Aparelhos Fabricantes",
      url: "https://api-teste.ieztelecom.net.br/api/v1/crm/aparelhos/fabricante/9f8276a1-0732-4d99-b91e-7b9f8e1a09d0",
    },
    {
      name: "Listar Tipos de Situacao",
      url: "https://api-teste.ieztelecom.net.br/api/v1/crm/situacao",
    },
    {
      name: "Listar Cidades",
      url: "https://api-teste.ieztelecom.net.br/api/v1/crm/cidade",
    },
    {
      name: "Ler Cep",
      url: "https://api-teste.ieztelecom.net.br/api/v1/crm/cidade/12245-730",
    },
    {
      name: "Listar Aparelhos",
      url: "https://api-teste.ieztelecom.net.br/api/v1/crm/aparelhos",
    },
    {
      name: "Listar Operadoras Movel",
      url: "https://api-teste.ieztelecom.net.br/api/v1/crm/operadorasmovel",
    },
    {
      name: "Obter Novo Sim Card",
      url: "https://api-teste.ieztelecom.net.br/api/v1/crm/simcard",
    },
    {
      name: "Obter Novo Sim Card Com Pin",
      url: "https://api-teste.ieztelecom.net.br/api/v1/crm/aparelhos/fabricantes",
    },
    {
      name: "Obter Novo Sim Card Com Puk",
      url: "https://api-teste.ieztelecom.net.br/api/v1/crm/simcard?pin2=4321&puk1=1234567890",
    },
    {
      name: "Solicitar Novo Numero",
      url: "https://api-teste.ieztelecom.net.br/api/v1/crm/numero/7818a245-c237-4e04-8476-ac3be263272f",
    },
    {
      name: "Listar Sim Cards Disponiveis",
      url: "https://api-teste.ieztelecom.net.br/api/v1/crm/simcards",
    },
  ];

  const headers = {
    headers: {
      Authorization: `Bearer ${token}`,
      "Content-Type": "application/json",
    },
  };

  // Cria um array de requisições para o http.batch
  const batchRequests = requests.map((req) => ["GET", req.url, null, headers]);

  // Executa todas as requisições em batch
  const responses = http.batch(batchRequests);

  // Verifica o status de cada resposta usando o nome da requisição
  responses.forEach((res, index) => {
    check(res, {
      [`${requests[index].name}: status is 200`]: (r) => r.status === 200,
    });
  });

  sleep(1);
}

export function handleSummary(data) {
  return {
    "relatorio.html": htmlReport(data),
  };
}
